...
weightedSumNE <- function (observed, weights, lag, funct_lag, par_lag, min_lag, max_lag , use_distr_lag, sum_up) #BJ
{
  dimY <- dim(observed)
  nTime <- dimY[1L]
  nUnits <- dimY[2L]
...
weightedSumAR <- function (observed, lag, funct_lag, par_lag, min_lag, max_lag, use_distr_lag, sum_up) #BJ
{
  dimY <- dim(observed)
  nTime <- dimY[1L]
  nUnits <- dimY[2L]
